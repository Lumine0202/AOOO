...
<body class="text-sm">
  ...
  <div>
    ...
    <form method="POST" action="{{ route('logout') }}">
      @csrf
      <a href="{{ route('logout') }}" onclick="event.preventDefault(); this.closest('form').submit();" class="text-white">
        {{ __('Log Out') }}
      </a>
    </form>
  </div>
      ...
      <form action="{{ route('groups.store') }}" method="POST" class="mb-6 form-section">
        ...
        <div class="mb-4">
          ...
          <input type="text" name="name" id="name" placeholder="Csoport neve" required
            class="mt-1 block w-full rounded-md shadow-sm border px-2 py-1" autocomplete="off">
        </div>
    ...
    <div id="add-item" class="hidden">
      ...
      <form action="{{ route('erettsegi.store') }}" method="POST" class="mb-6  form-section">
        ...
        <div class="mb-4">
          ...
          <textarea name="content" id="content" placeholder="Tartalom" required
            class="mt-1 block w-full rounded-md shadow-sm border px-2 py-1"></textarea>
        </div>
        @isset($groups)
        <div class="mb-4">
